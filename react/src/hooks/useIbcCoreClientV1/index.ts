...
export default function useIbcCoreClientV1() {
  ...
  const QueryVerifyMembership = ( options: any) => {
    const key = { type: 'QueryVerifyMembership',  };    
    return useQuery([key], () => {
      return  client.IbcCoreClientV1.query.queryVerifyMembership({...key}).then( res => res.data );
    }, options);
  }
  
  return {QueryClientState,QueryClientStates,QueryConsensusState,QueryConsensusStates,QueryConsensusStateHeights,QueryClientStatus,QueryClientParams,QueryUpgradedClientState,QueryUpgradedConsensusState,QueryVerifyMembership,
  }
}
