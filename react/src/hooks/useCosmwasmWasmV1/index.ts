...
export default function useCosmwasmWasmV1() {
  ...
  const QueryBuildAddress = (query: any, options: any) => {
    const key = { type: 'QueryBuildAddress', query };    
    return useQuery([key], () => {
      const {query } = key
      return  client.CosmwasmWasmV1.query.queryBuildAddress(query ?? undefined).then( res => res.data );
    }, options);
  }
  
  return {QueryContractInfo,QueryContractHistory,QueryContractsByCode,QueryAllContractState,QueryRawContractState,QuerySmartContractState,QueryCode,QueryCodes,QueryPinnedCodes,QueryParams,QueryContractsByCreator,QueryBuildAddress,
  }
}
