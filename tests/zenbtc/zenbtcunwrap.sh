...
case $environment in
    local|devnet|testnet)
        ...
        if [ "$environment" = "local" ]; then
            ...
            mint_address=$(zenrockd q zenbtc params --node $rpc_url | grep "mint_address:" | awk -F': ' '{print $2}' | tr -d ' ')
            program_id=$(zenrockd q zenbtc params --node $rpc_url | grep "program_id:" | awk -F': ' '{print $2}' | tr -d ' ')
            fee_wallet=$(zenrockd q zenbtc params --node $rpc_url | grep "fee_wallet:" | awk -F': ' '{print $2}' | tr -d ' ')
            multisig_key_address=$(zenrockd q zenbtc params --node $rpc_url | grep "multisig_key_address:" | awk -F': ' '{print $2}' | tr -d ' ')
...
echo "zenbtc balance: $(spl-token balance "$mint_address" --owner "$SIGNER_PUBKEY")"

# Prompt for unwrap amount
echo "Please enter the amount of zenbtc to unwrap:"
...
echo "enter destination address on bitcoin signet:"
read -r dest_address

echo "Unwrapping $unwrap_amount zenbtc from mint address: $mint_address"

# Capture the output and exit code
output=$(go run ./create-zenbtc-unwrap.go \
    --signer "$keypair_path" \
    --mint "$mint_address" \
    --program "$program_id" \
    --rpc "$solana_rpc" \
    ...
    --multisig-key-address "$multisig_key_address" 2>&1)
exit_code=$?
