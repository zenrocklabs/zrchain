...
func (k Keeper) Swaps(goCtx context.Context, req *types.QuerySwapsRequest) (*types.QuerySwapsResponse, error) {
	...
	swaps, pageRes, err := k.querySwap(goCtx, k.SwapsStore, req.Pagination, req.Creator, req.Pair, req.Workspace, req.SourceTxHash, req.Status, req.SwapId)
	if err != nil {
		return nil, err
	}
...
func (k Keeper) querySwap(goCtx context.Context, store collections.Map[uint64, types.Swap], pagination *query.PageRequest, creator, pair, workspace, sourceTxHash string, status types.SwapStatus, swapId uint64) ([]*types.Swap, *query.PageResponse, error) {
