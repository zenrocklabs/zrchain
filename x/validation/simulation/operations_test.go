...
func (s *SimTestSuite) TestSimulateMsgCreateValidator() {
	...
	require.NoError(err)
	require.Equal("zen1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7xc5y37", sdk.AccAddress(valaddr).String())
	require.Equal("zenvaloper1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7ves9zy", msg.ValidatorAddress)
...
func (s *SimTestSuite) TestSimulateMsgCancelUnbondingDelegation() {
	...
	udb := types.NewUnbondingDelegation(delegator.Address, val0bz, s.app.LastBlockHeight()+1, blockTime.Add(2*time.Minute), delTokens, 0, address.NewBech32Codec("zenvaloper"), address.NewBech32Codec("zen"))
...
func (s *SimTestSuite) TestSimulateMsgDelegate() {
	...
	require.NoError(err)
	require.True(operationMsg.OK)
	require.Equal("zen1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7xc5y37", msg.DelegatorAddress)
	require.Equal("stake", msg.Amount.Denom)
	require.Equal(sdk.MsgTypeURL(&types.MsgDelegate{}), sdk.MsgTypeURL(&msg))
	require.Equal("zenvaloper1tnh2q55v8wyygtt9srz5safamzdengsns4jcd6", msg.ValidatorAddress)
	require.Len(futureOperations, 0)
}
...
func (s *SimTestSuite) TestSimulateMsgUndelegate() {
	...
	require.NoError(err)
	require.True(operationMsg.OK)
	require.Equal("zen1ghekyjucln7y67ntx7cf27m9dpuxxemn044fej", msg.DelegatorAddress)
...
func (s *SimTestSuite) TestSimulateMsgBeginRedelegate() {
	...
	require.NoError(err)
	require.True(operationMsg.OK)
	require.Equal("zen1ua0fwyws7vzjrry3pqkklvf8mny93l9sl06wmy", msg.DelegatorAddress)
	require.Equal("stake", msg.Amount.Denom)
	require.Equal(sdk.MsgTypeURL(&types.MsgBeginRedelegate{}), sdk.MsgTypeURL(&msg))
	require.Equal("zenvaloper1ghekyjucln7y67ntx7cf27m9dpuxxemn953g2g", msg.ValidatorDstAddress)
	require.Equal("zenvaloper1p8wcgrjr4pjju90xg6u9cgq55dxwq8j7ves9zy", msg.ValidatorSrcAddress)
	require.Len(futureOperations, 0)
}
