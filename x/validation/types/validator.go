...
var _ ValidatorI = ValidatorHV{}

// NewValidator constructs a new Validator
func NewValidator(operator string, pubKey cryptotypes.PubKey, description Description) (ValidatorHV, error) {
	pkAny, err := codectypes.NewAnyWithValue(pubKey)
	if err != nil {
		return ValidatorHV{}, err
	}
...
type ValidatorsByVotingPower []ValidatorHV
...
func MustMarshalValidator(cdc codec.BinaryCodec, validator *ValidatorHV) []byte {
	return cdc.MustMarshal(validator)
}

// unmarshal a redelegation from a store value
func MustUnmarshalValidator(cdc codec.BinaryCodec, value []byte) ValidatorHV {
...
// unmarshal a redelegation from a store value
func UnmarshalValidator(cdc codec.BinaryCodec, value []byte) (v ValidatorHV, err error) {
	err = cdc.Unmarshal(value, &v)
	return v, err
}

// IsBonded checks if the validator status equals Bonded
func (v ValidatorHV) IsBonded() bool {
	return v.GetStatus() == Bonded
}

// IsUnbonded checks if the validator status equals Unbonded
func (v ValidatorHV) IsUnbonded() bool {
	return v.GetStatus() == Unbonded
}

// IsUnbonding checks if the validator status equals Unbonding
func (v ValidatorHV) IsUnbonding() bool {
...
func (v ValidatorHV) ABCIValidatorUpdate(r math.Int, power int64) abci.ValidatorUpdate {
...
func (v ValidatorHV) ABCIValidatorUpdateZero() abci.ValidatorUpdate {
	tmProtoPk, err := v.TmConsPublicKey()
	if err != nil {
		panic(err)
	}
...
func (v ValidatorHV) SetInitialCommission(commission Commission) (ValidatorHV, error) {
...
func (v ValidatorHV) InvalidExRate() bool {
	return v.TokensNative.IsZero() && v.DelegatorShares.IsPositive()
}

// calculate the token worth of provided shares
func (v ValidatorHV) TokensFromShares(shares math.LegacyDec) math.LegacyDec {
	return (shares.MulInt(v.TokensNative)).Quo(v.DelegatorShares)
}

// calculate the token worth of provided shares, truncated
func (v ValidatorHV) TokensFromSharesTruncated(shares math.LegacyDec) math.LegacyDec {
	return (shares.MulInt(v.TokensNative)).QuoTruncate(v.DelegatorShares)
}

// TokensFromSharesRoundUp returns the token worth of provided shares, rounded
// up.
func (v ValidatorHV) TokensFromSharesRoundUp(shares math.LegacyDec) math.LegacyDec {
	return (shares.MulInt(v.TokensNative)).QuoRoundUp(v.DelegatorShares)
}

// SharesFromTokens returns the shares of a delegation given a bond amount. It
// returns an error if the validator has no tokens.
func (v ValidatorHV) SharesFromTokens(amt math.Int) (math.LegacyDec, error) {
	if v.TokensNative.IsZero() {
...
func (v ValidatorHV) SharesFromTokensTruncated(amt math.Int) (math.LegacyDec, error) {
	if v.TokensNative.IsZero() {
		return math.LegacyZeroDec(), ErrInsufficientShares
	}
...
func (v ValidatorHV) BondedTokens() math.Int {
	if v.IsBonded() {
		return v.TokensNative
	}
...
func (v ValidatorHV) ConsensusPower(r math.Int) int64 {
	if v.IsBonded() {
...
func (v ValidatorHV) PotentialConsensusPower(r math.Int) int64 {
	return sdk.TokensToConsensusPower(v.TokensNative, r)
}

// ConsensusPower gets the consensus-engine power. Aa reduction of 10^6 from
// validator tokens is applied
func (v ValidatorHV) AVSConsensusPower(r math.Int) int64 {
	if v.IsBonded() {
		return v.AVSPotentialConsensusPower(r)
	}
...
// PotentialConsensusPower returns the potential consensus-engine power.
func (v ValidatorHV) AVSPotentialConsensusPower(r math.Int) int64 {
	return sdk.TokensToConsensusPower(v.TokensAVS, r)
}

// UpdateStatus updates the location of the shares within a validator
// to reflect the new status
func (v ValidatorHV) UpdateStatus(newStatus BondStatus) ValidatorHV {
	v.Status = newStatus
	return v
}

// AddTokensFromDel adds tokens to a validator
func (v ValidatorHV) AddTokensFromDel(amount math.Int) (ValidatorHV, math.LegacyDec) {
	...
	v.TokensNative = v.TokensNative.Add(amount)
...
func (v ValidatorHV) RemoveTokens(tokens math.Int) ValidatorHV {
	if tokens.IsNegative() {
		panic(fmt.Sprintf("should not happen: trying to remove negative tokens %v", tokens))
	}

	if v.TokensNative.LT(tokens) {
		panic(fmt.Sprintf("should not happen: only have %v tokens, trying to remove %v", v.TokensNative, tokens))
	}

	v.TokensNative = v.TokensNative.Sub(tokens)
...
func (v ValidatorHV) RemoveDelShares(delShares math.LegacyDec) (ValidatorHV, math.Int) {
	remainingShares := v.DelegatorShares.Sub(delShares)

	var issuedTokens math.Int
	if remainingShares.IsZero() {
		// last delegation share gets any trimmings
		issuedTokens = v.TokensNative
		v.TokensNative = math.ZeroInt()
	} else {
		// leave excess tokens in the validator
		// however fully use all the delegator shares
		issuedTokens = v.TokensFromShares(delShares).TruncateInt()
		v.TokensNative = v.TokensNative.Sub(issuedTokens)

		if v.TokensNative.IsNegative() {
...
// validators.
func (v *ValidatorHV) MinEqual(other *ValidatorHV) bool {
	return v.OperatorAddress == other.OperatorAddress &&
		v.Status == other.Status &&
		v.TokensNative.Equal(other.TokensNative) &&
...
func (v *ValidatorHV) Equal(v2 *ValidatorHV) bool {
	return v.MinEqual(v2) &&
		v.UnbondingHeight == v2.UnbondingHeight &&
		v.UnbondingTime.Equal(v2.UnbondingTime)
}

func (v ValidatorHV) IsJailed() bool        { return v.Jailed }
func (v ValidatorHV) GetMoniker() string    { return v.Description.Moniker }
func (v ValidatorHV) GetStatus() BondStatus { return v.Status }
func (v ValidatorHV) GetOperator() string {
	return v.OperatorAddress
}

// ConsPubKey returns the validator PubKey as a cryptotypes.PubKey.
func (v ValidatorHV) ConsPubKey() (cryptotypes.PubKey, error) {
...
func (v ValidatorHV) TmConsPublicKey() (cmtprotocrypto.PublicKey, error) {
	return v.CmtConsPublicKey()
}

// CmtConsPublicKey casts Validator.ConsensusPubkey to cmtprotocrypto.PubKey.
func (v ValidatorHV) CmtConsPublicKey() (cmtprotocrypto.PublicKey, error) {
	pk, err := v.ConsPubKey()
	if err != nil {
...
func (v ValidatorHV) GetConsAddr() ([]byte, error) {
	pk, ok := v.ConsensusPubkey.GetCachedValue().(cryptotypes.PubKey)
	if !ok {
		return nil, errors.Wrapf(sdkerrors.ErrInvalidType, "expecting cryptotypes.PubKey, got %T", pk)
	}
...
func (v ValidatorHV) GetTokens() math.Int       { return v.TokensNative }
func (v ValidatorHV) GetBondedTokens() math.Int { return v.BondedTokens() }
func (v ValidatorHV) GetConsensusPower(r math.Int) int64 {
	return v.ConsensusPower(r)
}
func (v ValidatorHV) GetCommission() math.LegacyDec      { return v.Commission.Rate }
func (v ValidatorHV) GetMinSelfDelegation() math.Int     { return v.MinSelfDelegation }
func (v ValidatorHV) GetDelegatorShares() math.LegacyDec { return v.DelegatorShares }

// UnpackInterfaces implements UnpackInterfacesMessage.UnpackInterfaces
func (v ValidatorHV) UnpackInterfaces(unpacker codectypes.AnyUnpacker) error {
