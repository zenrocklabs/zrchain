...
func NewHistoricalInfo(header cmtproto.Header, valSet Validators, powerReduction math.Int) HistoricalInfoHV {
...
func MustUnmarshalHistoricalInfo(cdc codec.BinaryCodec, value []byte) HistoricalInfoHV {
...
func UnmarshalHistoricalInfo(cdc codec.BinaryCodec, value []byte) (hi HistoricalInfoHV, err error) {
	err = cdc.Unmarshal(value, &hi)
	return hi, err
}

// ValidateBasic will ensure HistoricalInfo is not nil and sorted
func ValidateBasic(hi HistoricalInfoHV, valAc address.Codec) error {
	if len(hi.Valset) == 0 {
...
func (hi *HistoricalInfoHV) Equal(hi2 *HistoricalInfoHV) bool {
...
func (hi HistoricalInfoHV) UnpackInterfaces(c codectypes.AnyUnpacker) error {
	for i := range hi.Valset {
