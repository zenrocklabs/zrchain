...
1. **Run Race Detector**: Execute `go test -race ./sidecar/...` to confirm the race and check for other issues
2. **Implement Fix**: Add mutex protection to all stateCache operations
3. **Add Test Coverage**: Create test that triggers concurrent stateCache access to verify fix
4. **Code Review**: Get second set of eyes on the fix and broader concurrency patterns

### Testing Strategy

1. **Race Detector Tests**: 
   - Run existing tests with `-race` flag
   - Add concurrent access test specifically for stateCache operations
   - Verify no races detected after fix
