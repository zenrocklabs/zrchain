...
func (inst InitializeMultisig) Build() *token.Instruction {
	return &token.Instruction{BaseVariant: ag_binary.BaseVariant{
		Impl:   inst,
		TypeID: ag_binary.TypeIDFromUint8(token.Instruction_InitializeMultisig),
	}}
}
...
func (inst InitializeMultisig) ValidateAndBuild() (*token.Instruction, error) {
...
func (inst *InitializeMultisig) Validate() error {
	...
	// Check whether all (required) accounts are set:
	{
		...
		if len(inst.Signers) > token.MAX_SIGNERS {
...
func (inst *InitializeMultisig) EncodeToTree(parent ag_treeout.Branches) {
	parent.Child(ag_format.Program(token.ProgramName, token.ProgramID)).
		//
